...
POSITION_COORDS = {
    "home": [417.61, -0.76, 477.45, 174.25, 179.99, -7.65],
    # 2026-07-13 임시 진단용: 원래 scan=[560.37, 256.18, 460.56, 44.09, 175.79, -130.28],
    # scan_b=[560.37, -121.8, ...] (그 뒤 회피 테스트용으로 Y=-300까지 늘렸던 것도 포함,
    # 전부 원복). 이번엔 스윕 자체를 과거 target1 위치 상공 -> target3 방향으로 재배치 --
    # X/Y만 각각 target1/target3의 X/Y로 바꾸고, Z(460.56)/자세(44.09,175.79,-130.28)는
    # 기존 scan의 검증된 카메라 프레이밍 값을 그대로 유지(놓는 높이 271mm보다 190mm 위라
    # 여유는 충분). 테스트 끝나면 원래 scan/scan_b 값으로 되돌릴 것.
    "scan": [200.0, 100.0, 460.56, 44.09, 175.79, -130.28],
    # 2026-07-09: 스캔 스윕(scan -> scan_b) 범위. 실측 완료 — Y축으로만 이동.
    "scan_b": [199.93, -100.092, 460.56, 44.09, 175.79, -130.28],
    # 2026-07-13 임시 진단용: 원래 target1 = [200.0, 100.0, 271.058, 138.332, -179.994, -43.561]
    # (TCP x=0.20m -- dsr_policy_path.py 캐비어트 2의 "low-X 취약구간(x<0.25m)"에 걸림).
    # 장애물 회피 통합 후 목표 도달 실패 원인이 그 취약구간 때문인지, obstacle observation
    # 배선 문제인지 분리하려고 x/y만 바꿔서 TCP x=0.30m로 옮기고 두 장애물과도 거리를
    # 벌려둔 테스트 좌표로 임시 교체 -- 테스트 끝나면 위 원본 값으로 되돌릴 것.
    "target1": [300.0, 250.0, 271.058, 138.332, -179.994, -43.561],
