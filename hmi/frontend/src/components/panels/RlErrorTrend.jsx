...
const W = 640, DEFAULT_H = 220, PAD_L = 52, PAD_R = 16, PAD_T = 18, PAD_B = 36;
  ...
  return (
    <div className="card">
      ...
      <svg
        ...
        role="img"
        aria-label="한 시나리오 내 스텝별 목표 거리 오차 추이"
      >
        ...
        <g fill="var(--muted)" fontSize="13" textAnchor="end">
          {yTicks.map((v) => <text key={v} x={PAD_L - 8} y={y(v) + 4}>{v}</text>)}
        </g>
        <line
          x1={PAD_L} y1={y(goalThresholdMm)} x2={W - PAD_R} y2={y(goalThresholdMm)}
          stroke="var(--warn)" strokeWidth="1.4" strokeDasharray="5 4"
        />
        <text x={W - PAD_R} y={y(goalThresholdMm) - 6} fill="var(--warn)" fontSize="13.5" fontWeight="700" textAnchor="end">
        ...
        <text x={W / 2} y={H - 8} fill="var(--muted)" fontSize="13.5" textAnchor="middle">
